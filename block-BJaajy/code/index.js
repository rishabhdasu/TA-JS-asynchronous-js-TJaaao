...
let userName = ["Aditya17Singh", "prank7", "getify", "piranha", "gaearon"];

let usernamePromise = userName.map((user) => {
  return fetch(`https://api.github.com/users/${user}`).then((res) =>
    res.json()
  );
});
Promise.all(usernamePromise)
  .then((u) => {
    u.forEach((elm) => console.log(elm.followers));
  })
...
How much time will it take for the promise to resolve?
*/
